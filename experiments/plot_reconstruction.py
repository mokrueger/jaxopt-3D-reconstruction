...
def setup_scene(floor_z=None):
    ...
    if floor_z is not None:
        ...
        floor = bplt.Scatter(
            [0, 0, floor_z],
            marker_type="cubes",
            size=(floor_size, floor_size, 0.1),
            name="floor",
        )
...
def set_color(mesh, color):
    ...
    mesh.materials[0].node_tree.nodes["Principled BSDF"].inputs[
        "Base Color"
    ].default_value = color
...
def plot_cameras(
    ...
    frustum_points = np.vstack(
        [
            np.einsum(
                "ij,...j->...i",
                poses[i].R @ np.linalg.inv(intrinsics[i]),
                np.array(
                    [
                        [0, heights[i], 1],
    ...
    frustum_edges = np.vstack(
        [
            np.array([[0, 1], [1, 3], [3, 2], [2, 0], [0, 4], [1, 4], [2, 4], [3, 4]])
            + i * 5
            for i in range(len(intrinsics))
        ]
    )
    ...
    mesh.from_pydata(
        frustum_points, frustum_edges, frustum_faces
    )  # , frustum_edges, frustum_faces)
    ...
    mesh_fill.from_pydata(
        np.vstack([frustum_points, np.zeros(3)]), frustum_edges, fill_faces
    )
