...
class GtsamBundleAdjustmentBenchmark(BundleAdjustmentBenchmark):
    ...
    def benchmark(self, args=None):
        ...
        for index, de in enumerate(self.dataset.datasetEntries, start=0):
            ...
            for j, p2d_p3d in enumerate(de.map2d_3d(self.dataset.points3D_mapped, zipped=True), start=0):
            ...
            corresponding_cameras.update({
                de.identifier:
                    Camera(
                        ...
                        camera_intrinsics=params_to_intrinsics(fx=camera_intrinsics.fx(),
        ...
        for p in self.dataset.points3D:
            ...
            corresponding_points.update({
                p.identifier: Point3D(identifier=p.identifier,
                                      x=new_point[0],
                                      y=new_point[1],
                                      z=new_point[2],
                                      metadata={**p.metadata, "note": "has been copied from original"})
            })
